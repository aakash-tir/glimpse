...
- **Coverage now measures `src/main`.** The old exclude made the headline number describe only the renderer + shared layers. Honest baseline **82.89 % lines / 81.13 % statements**.
...
_Closed 2026-08-12. Every automated gate passed on the branch, again on `main` post-merge, and on CI. The alert feature was verified end to end against a live Environment Canada warning, and the manual visual sweep — the milestone's real risk, since the Tailwind removal touched every surface and no automated test asserts a visual property — was signed off the same day. A few non-blocking checks remain listed in [`review-findings.md`](./review-findings.md)._

_Post-merge, CI caught two M3 window-mode E2E failures that pass on the development machine: `dragWindowToMiddle` used a hard-coded −1100 px drag that clamps to the left edge on a narrower display. Display-dependent since M3, surfaced only once M11's CI ran the specs somewhere other than this desk — which is the point of having it. Fixed in `1b7ca0c`._
