...
Sad-cloud icon at the regular 64 × 64 size. The sad cloud is the only signal — there is no hover tooltip (the icon's 96 × 96 BrowserWindow is too narrow to render the tooltip text without DWM clipping, and the sad face alone reads as "something's wrong"). Single-click on the sad cloud is **ignored** (no expand): there's nothing useful to show in window mode while the data layer can't reach a forecast, and it spares the user a confusing screen of skeleton placeholders. Double-click into drag mode still works so the icon is repositionable while offline. The user is forewarned about this state in the [onboarding tutorial](./onboarding.md) (offline-state preview step). Retry uses **exponential backoff doubling 5 → 10 → 20 → 40 → 60 min, capped at 1 h**, while the app is active. Backoff resets to 5 min only on a successful fetch. (See [data-sources.md](./data-sources.md) for active-state and refresh details.)
