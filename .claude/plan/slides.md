...
- **Scroll affordance:** **white edge-fade glow** — ~14 px wide semi-transparent white **radial gradient** anchored at the panel-edge midpoint on whichever side(s) have more content to scroll to. The radial origin produces curved iso-alpha lines that arc inward from the edge: brightest at the edge, fading transparent toward the cells. Disappears at the boundary. Sits just inside the side padding so it overlays the first / last visible cell rather than the empty padding zone.
