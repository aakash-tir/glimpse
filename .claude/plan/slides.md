...
- **Scroll affordance:** **white edge-fade gradient** — 24 px semi-transparent white gradient on the right edge whenever there's more content to scroll to (and on the left edge once the user has scrolled past the start). Fade highlights the scrollable direction with a soft white glow against the dark slide; disappears at the boundary. Sits just inside the side padding so it overlays the first / last visible cell rather than the empty padding zone.
