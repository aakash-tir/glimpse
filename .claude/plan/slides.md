...
- **Scroll affordance:** **white edge-fade glow** — ~14 px semi-transparent white gradient on the right edge whenever there's more content to scroll to (and on the left edge once the user has scrolled past the start). Alpha follows a **quadratic curve** (concentrated near the panel edge, mostly transparent toward the cells) so the highlight reads as a soft glow rather than a uniform stripe. Disappears at the boundary. Sits just inside the side padding so it overlays the first / last visible cell rather than the empty padding zone.
